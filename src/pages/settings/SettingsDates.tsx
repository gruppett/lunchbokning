  ...
  return (
    <div className="flex gap-3 p-3 bg-slate-50 sm:w-max flex-col">
      <div className="">
            ...
            <table className="table-auto text-left border-collapse">
              ...
              <tbody>
                {periodData?.map((i: any, key: Key) => (
                  <tr
                    className="bg-white even:bg-slate-50 cursor-pointer hover:bg-slate-100"
                    onClick={() => selectPeriod(key as number)}
                    key={key}
                  >
        ...
        {isPeriodSelected ? (
          <div className="min-content">
            <button className="px-3 py-1 bg-red-500 hover:bg-opacity-60">
              Radera {periodData[selectedPeriod].periodName}
            </button>
          </div>
        ) : (
          ...
          <table className="table-auto text-left border-collapse">
            ...
            <tbody>
              {excludedData?.map((i: any, key: Key) => (
                <tr
                  className="bg-white even:bg-slate-50 cursor-pointer hover:bg-slate-100"
                  onClick={() => selectExcluded(key as number)}
                  key={key}
                >
      ...
      {isExcludedSelected ? (
        <div className="min-content">
          <button className="px-3 py-1 bg-red-500 hover:bg-opacity-60">
            Radera {excludedData[selectedExcluded].name}
          </button>
        </div>
      ) : (
